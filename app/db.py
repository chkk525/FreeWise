...
def ensure_schema_migrations(engine=None) -> None:
    ...
    with engine.begin() as conn:
        ...
        # ── ApiToken.scopes column + backfill ────────────────────────────
        # Phase E of the kindle browser-extension PR. The "scopes" column
        # is comma-separated; ``require_scope`` in app/api_v2/auth.py
        # short-circuits on NULL (legacy full-access). After this migration
        # NO production row should be NULL — the backfill below assigns a
        # reasonable default (full Readwise-compat + new kindle:import) so
        # existing tokens keep working AND the dependency is fail-closed
        # for any token that explicitly carries ``scopes=""``.
        if token_cols and "scopes" not in token_cols:
            _log.info("migration: adding apitoken.scopes column")
            conn.execute(text("ALTER TABLE apitoken ADD COLUMN scopes VARCHAR"))
        # Backfill is idempotent — only touches rows where scopes IS NULL.
        # Runs every startup but skipped on partial-schema DBs (the
        # pytest fixture creates ``book`` only); cheap UPDATE that
        # touches 0 rows after the first deploy.
        if token_cols:
            _DEFAULT_SCOPES = (
                "kindle:import,highlights:read,highlights:write,books:read"
            )
            backfilled_scopes = conn.execute(
                text("UPDATE apitoken SET scopes = :s WHERE scopes IS NULL"),
                {"s": _DEFAULT_SCOPES},
            ).rowcount
            if backfilled_scopes:
                _log.info(
                    "migration: backfilled scopes on %d pre-existing apitoken row(s)",
                    backfilled_scopes,
                )
